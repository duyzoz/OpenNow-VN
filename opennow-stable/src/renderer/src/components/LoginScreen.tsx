...
import { LogIn, ChevronDown, QrCode, User } from "lucide-react";
    ...
    <div className="login-screen">
      ...
      <m.div className="login-content" {...dialogMotion}>
        ...
        <div className="login-card">
          ...
          {savedAccounts.length > 0 && !isQrLoginActive && (
            <div className="login-saved-accounts">
              <div className="login-saved-accounts-label">{t("auth.savedAccounts.title")}</div>
              <div className="login-saved-accounts-list">
                {savedAccounts.map((account) => (
                  <button
                    key={account.userId}
                    className="login-saved-account-btn"
                    onClick={() => onQuickLogin?.(account.userId)}
                    disabled={isLoading || isInitializing}
                    type="button"
                  >
                    <div className="login-saved-account-avatar">
                      {account.avatarUrl ? (
                    ...
                    <div className="login-saved-account-info">
                      <span className="login-saved-account-name">{account.displayName}</span>
                      {account.providerName && (
                        <span className="login-saved-account-provider">{account.providerName}</span>
                      )}
                    </div>
                  </button>
                ))}
              </div>
              <div className="login-saved-accounts-divider">
                <span>{t("auth.savedAccounts.orSignInNew")}</span>
              </div>
            </div>
          )}
