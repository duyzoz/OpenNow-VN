...
export const SearchSuggestions = memo(function SearchSuggestions({
  ...
  const formatRecentAccess = (value: string): string => {
    const timestamp = new Date(value).getTime();
    if (!Number.isFinite(timestamp)) return "";
    const elapsed = Math.max(0, Date.now() - timestamp);
    if (elapsed < 60_000) return t("search.suggestions.justNow");
    if (elapsed < 3_600_000) {
      return t("search.suggestions.minutesAgo", { count: Math.max(1, Math.round(elapsed / 60_000)) });
    }
    if (elapsed < 86_400_000) {
      return t("search.suggestions.hoursAgo", { count: Math.max(1, Math.round(elapsed / 3_600_000)) });
    }
    if (elapsed < 7 * 86_400_000) {
      return t("search.suggestions.daysAgo", { count: Math.max(1, Math.round(elapsed / 86_400_000)) });
    }
    return t("search.suggestions.accessedAt", {
