...
export function SettingsNativeStreamerSection({
  ...
  useEffect(() => {
    // PERF: never probe during the commit that opens this tab. Kicking the probe
    // off inline made the whole window freeze while GStreamer's runtime was
    // discovered. We now let the tab paint first, then probe when the main thread
    // is idle. Main-side caching makes every subsequent open instant.
    let cancelled = false;
    const run = (): void => {
      if (!cancelled) void refreshNativeStreamerStatus();
    };
