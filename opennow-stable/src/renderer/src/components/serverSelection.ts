...
export function recordServerRouteHealth(
  ...
  try {
    ...
    const poor = health.rttMs >= 300 || health.jitterMs >= 20 || health.packetLossPercent >= 1;
    const next: ServerSelectionTelemetry = {
      ...
      observedAtMs: nowMs,
      poorSamples: poor ? route.poorSamples + 1 : route.poorSamples,
      goodSamples: poor ? route.goodSamples : route.goodSamples + 1,
    };
...
export function getServerRouteAdvice(route: ServerSelectionTelemetry | null, nowMs: number = Date.now()): ServerRouteAdvice {
  ...
  if (route.poorSamples >= 3) return "avoid";
  if (route.goodSamples >= 3) return "healthy";
  return "unknown";
}
