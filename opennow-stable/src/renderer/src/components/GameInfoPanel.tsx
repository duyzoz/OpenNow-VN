...
function getCachedDescription(game: GameInfo): string {
  const cached = descCache.get(game.id);
  if (cached) return cached;
  const description = getCatalogDescription(game);
...
export function GameInfoPanel({
  ...
  useEffect(() => {
    if (!game) return;
    setDesc(getCachedDescription(game));
    setHeroIndex(0);
    setImgErr(false);
    setCoverErr(false);
          ...
          <div className="game-info-meta">
            ...
            <div className="game-info-actions">
              {isActiveGame ? (
                ...
                <button className="game-info-btn game-info-btn--play" onClick={() => onPlay?.(selectedVariantId)}>
