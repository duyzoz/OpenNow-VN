...
// Additive (v9): custom in-app popup that replaces the native OS "close
// window" confirm dialog. Only ever mounted in the main window (see call
// site below). Self-contained: it owns its own IPC subscription and state,
// so it can never affect anything else in the app tree if it fails.
function CloseChoiceModal(): JSX.Element | null {
  const { t } = useTranslation();
  const [request, setRequest] = useState<{ hasStreamWindow: boolean } | null>(null);

  useEffect(() => {
    if (typeof window === "undefined" || !window.openNow?.onRequestMainWindowCloseChoice) return;
    const unsubscribe = window.openNow.onRequestMainWindowCloseChoice((payload) => {
      setRequest(payload);
    });
    return () => {
      try {
  ...
  const respond = useCallback((choice: "tray" | "quit" | "cancel") => {
    setRequest(null);
    try {
      window.openNow?.respondMainWindowCloseChoice?.(choice);
    } catch (error) {
      console.warn("[CloseChoiceModal] Failed to send close choice response:", error);
    }
  }, []);
  ...
  return (
    <ModalSurface
      ...
      onConfirm={() => respond("tray")}
      motion="compact"
      overlayClassName="close-choice-confirm"
      backdropClassName="close-choice-confirm-backdrop"
      panelClassName="close-choice-confirm-card"
      ariaLabel={t("app.closeChoice.title")}
      backdropLabel={t("app.actions.cancel")}
    >
      <div className="close-choice-confirm-icon" aria-hidden="true">
        <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
          <path d="M18.36 6.64A9 9 0 1 1 5.64 6.64" />
          <line x1="12" y1="2" x2="12" y2="12" />
        </svg>
      </div>
      <div className="close-choice-confirm-kicker">OpenNOW</div>
      <h3 className="close-choice-confirm-title">{t("app.closeChoice.title")}</h3>
      <p className="close-choice-confirm-text">
        {request?.hasStreamWindow ? t("app.closeChoice.descriptionWithStream") : t("app.closeChoice.description")}
      </p>
      <div className="close-choice-confirm-actions">
        <button
          type="button"
          className="close-choice-confirm-btn close-choice-confirm-btn-tray"
        ...
        >
          ...
          {t("app.closeChoice.minimizeToTray")}
        </button>
        <button
          type="button"
          className="close-choice-confirm-btn close-choice-confirm-btn-quit"
          onClick={() => respond("quit")}
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
          {t("app.closeChoice.quitApp")}
        </button>
        <button
          type="button"
          className="close-choice-confirm-btn close-choice-confirm-btn-cancel"
...
export function App(): JSX.Element {
  ...
  // Additive (v9+): cloud client overlay states
  ...
  const markDiscordStreamStarted = useCallback((): void => {
    ...
    const gameName = (streamingGameRef.current?.title || "Streamed via OpenNow").trim();
  ...
  // Additive (v9): auto-launch the requested game once, only inside the
  // dedicated stream window. This never runs in the main window because
  ...
  const handleStreamShortcutAction = useCallback((action: "toggleStats" | "togglePointerLock"): void => {
    switch (action) {
      case "toggleStats":
  ...
  }, [requestPointerLockCapture, streamStatus]);
          ...
          >
            {mainPage === "home" && (
              <HomePage
                ...
                onResumeGame={() => { void handleResumeFromNavbar(); }}
                onTerminateGame={() => { void handleTerminateNavbarSession(); }}
              />
            )}
